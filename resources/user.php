	...
	public function __construct($data=null) {
		...
		function gigya_validate_email($valid,$email,$error){
			if($valid) {
				...
				if(empty($email)) {
					return new WP_Error('error',__("<strong>ERROR: </strong>Please provide an email address"));
				} else {
					return new WP_Error('error',__("<strong>ERROR: </strong>Invalid email address"));
				}
			} 
		}
...
//		// if exist - need to ask user if already registered - create new account or link account
	...
	public function register_email($email){
		...
		if(is_wp_error($is_email)) 
			//return wp_send_json_error ($is_email->;
		// check if email doesnt belong to site user
		$user_id = email_exists($email);
		if($user_id) 
            return wp_send_json_error(array('error' => "<strong>ERROR: </strong>".__('The email you provided is already used, Please provide a different email or link to an existing account')));
			//return new WP_Error('error',"<strong>ERROR: </strong>".__('The email you provided is already used, Please provide a different email or link to an existing account'));
	...
	public function link_account($email,$password="",$force_login = 0){
		...
		if(empty($password)) 
			return new WP_Error('error',"<strong>ERROR: </strong>".__('Missing Password!')); 
		
		if(!wp_check_password( $password,$user->user_pass,$user_id))
			return new WP_Error('error',"<strong>ERROR: </strong>".__('Wrong Password, Please try again'));
		// login user to site
    ...
    private function gen_reg_form($email = '') {
        ob_start();
        ?>
        <form name="registerform" class="gigya-reg-form" id="registerform" action="<?php echo esc_url( site_url('wp-login.php?action=register', 'login_post') ); ?>" method="post">
	<p>
		<label for="user_login"><?php _e('Username') ?><br />
		<input type="text" name="user_login" id="user_login" class="input" value="<?php echo $this->generate_user_name($this->data->user->nickname) ?>" size="20" /></label>
	</p>
	<p>
		<label for="user_email"><?php _e('E-mail') ?><br />
		<input type="text" name="user_email" id="user_email" class="input" value="<?php echo $this->generate_email($email); ?>" size="25" /></label>
	</p>
<?php do_action('register_form'); ?>
	<p id="reg_passmail"><?php _e('A password will be e-mailed to you.') ?></p>
	<br class="clear" />
	<p class="submit"><input type="submit" name="wp-submit" id="gigya-ajax-submit" class="button button-primary button-large" value="<?php esc_attr_e('Register'); ?>" /></p>
</form>
<?php
        return ob_get_clean();
    }
